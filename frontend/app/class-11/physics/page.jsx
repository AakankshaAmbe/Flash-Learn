...
export default function DynamicTopicPage({ classLevel, subject }) {
  ...
  const fetchTopic = async () => {
    ...
    try {
      ...
      // Find topic using normalized comparison
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-white/70">
              Class {classLevel} {subject.charAt(0).toUpperCase() + subject.slice(1)}
            </p>
          </div>
        </div>
      </motion.header>
      ...
      <div className="flex h-[calc(100vh-100px)]">
        ...
        <motion.div className="w-[70%] p-6 overflow-y-auto">
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 mb-6 border border-white/20">
            <h2 className="text-3xl font-bold text-white mb-4">{topicData.topic_name}</h2>
            <p className="text-white/80 text-lg leading-relaxed mb-6">{topicData.description}</p>
        ...
        <motion.div className="w-[30%] p-6 border-l border-white/10">
          ...
          <div className="space-y-4">
            <button
              onClick={() => router.push(`/class-${classLevel}/${subject}/flashcards?topicId=${topicData.id}`)}
            ...
            <button
              onClick={() => router.push(`/class-${classLevel}/${subject}/quiz?topicId=${topicData.id}`)}
            ...
            <button
              onClick={() => router.push(`/class-${classLevel}/${subject}/3d_model?topicId=${topicData.id}`)}
            ...
            <div className="mt-8 bg-white/5 backdrop-blur-lg rounded-xl p-4 border border-white/10">
              ...
              <div className="space-y-2 text-sm text-white/70">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-yellow-400">{topicData.difficulty || "N/A"}</span>
                </div>
                <div className="flex justify-between">
                  <span>Progress:</span>
                  <span className="text-teal-400">{topicData.progress || "0"}%</span>
                </div>
              </div>
            </div>
          </div>
